...
| Date | Changes |
|------|---------|
| Feb 7, 2026 | Hero choreography rewrite per strict spec: full-width dark hero on entry (no split), global white wipe at 10–20% covers entire viewport (left+right), 3-sided gradient border (top, left, bottom — right OPEN, matches Infotron logo) with soft blurred glow trails (no grids/particles) staggers upward at 20–40%, controlled scale 45–75%, takeover 75–100%. rAF smoothing factor raised from 0.12 → 0.20 for snappier response. |
...
| May 16, 2026 | Multi-task: (1) Added "AI Engineers On Demand" premium highlight badge at top of new Tech Ecosystem section — glassmorphism panel with breathing aurora, "RECOMMENDED BY INDUSTRY LEADERS" micro-label, animated orbit icon (cyan core + rotating rings + violet spark), Playfair italic-gradient title, subtitle, and "Talk to delivery" CTA pill. (2) Mobile dropdown fixed: Managed Services row split into a Link (label → navigates) + separate chevron Button (toggles submenu). Verified: label click → `/services/managed-services`, chevron click → URL unchanged + submenu expands. (3) Favicon updated to `favicon.png` + `apple-touch-icon.png` (180x180) using gradient logo source. (4) Logo across site replaced with new full-white version `/logo-white.png` (Header + Footer). (5) Contact page locations redesigned as 3 premium glassmorphism cards in `lg:grid-cols-3` grid (US/UK/India) with eyebrow pill, Playfair italic-gradient heading "Where we *deliver.*", flag glass tiles, region eyebrow, country names, and tabbed address list with cyan-violet bullet dots. Added UK office: London / 71–75 Shelton Street, Covent Garden, WC2H 9JQ. Removed Canada. (6) Homepage section reorder after hero: Tech Ecosystem (now standalone with AI Engineers badge) → Build-Operate-Transfer → Industries We Serve (standalone) → Engagement Models. Verified via runtime y-coordinates: Tech 3780 < BOT 5253 < Industries 6032 < Engagement 6817. |
| May 15, 2026 | Engagement reveal re-trigger: Removed one-shot `unobserve`. Single observer now uses thresholds `[0, 0.25]` — adds `.is-revealed` when ratio ≥ 0.25, removes it only when card is fully out of viewport (ratio === 0). Effect re-plays the staggered fold-open every time the cards re-enter view on scroll up or scroll down. Verified by 2-pass scroll telemetry capturing the reset + re-reveal mid-stagger. |
