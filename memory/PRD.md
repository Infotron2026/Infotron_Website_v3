...
| Date | Changes |
|------|---------|
| Feb 7, 2026 | Hero refined per choreography spec: hard 50%/50% boundary (no white bleed), brand pink→purple→blue gradient border framing INFOTRON (matches Infotron logo). 6-stage narrative: 0–15% calm entry, 15–30% border lines stagger upward + fade, 0–35% build-up grid translates upward, 30–45% transition + subtle pre-scale, 45–75% controlled scale (transform-origin centre), 75–100% takeover (mask dissolves → full-screen video). Hero height 180vh→250vh for deliberate scroll capture. SVG-native transform attribute for reliable scale. |
