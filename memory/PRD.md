...
| May 15, 2026 | Premium separator heading added between Industries marquee and the Tech Ecosystem: animated cyan→violet shimmering gradient hairline with a centered glassmorphism dot (multi-ring cyan/violet halo, 3.4s breath pulse), Playfair Display title "Technology *Ecosystem*" (italic-gradient on the second word, 5s breathing text-glow), and subtitle "Platforms powering modern digital transformation." Eyebrow chip "TECHNOLOGY ECOSYSTEM" removed from the center panel since the new section heading is now the canonical title. Reduced-motion suppresses the shimmer, dot pulse, and glow. |
