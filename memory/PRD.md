...
| May 15, 2026 | Tech Ecosystem fine-tuning: (1) OpenAI + AWS SVG fills patched — OpenAI gained `fill="#FFFFFF"` on root `<svg>`, AWS wordmark gray `#252f3e` swapped to white via sed (orange `#f90` preserved). Both logos now legible on dark bg. (2) Marquee edge-fade mask extended from 8% → 14% on each side so chips dissolve smoothly into the void instead of clipping at row edges. (3) Subtle pulse halo on the chip mirrored in the center panel — `.is-active` class adds a cyan ring (`box-shadow` rings) + 2.6s `tech-eco-active-pulse` keyframe that breathes between 16px and 28px outer glow + 0.40→0.65 opacity. Passed via React prop (`isActive={(i % rowLength) === activeIdx}`) to both copies of each chip. |
