...
| May 15, 2026 | Managed Services page — premium offerings nav added between Hero and Key Benefits: 3 glassmorphism cards linking to `/services/managed-services/oracle-ams`, `/services/managed-services/infrastructure-it-operations`, `/services/managed-services/microsoft-workplace-support` (mirrors the Header side-flyout exactly). Each card: cyan tag pill, rotating arrow circle top-right, 56px cyan-violet icon tile, bold title, cyan tagline (`ERP · HCM · SCM` etc.), description, and animated "Explore offering" CTA. Hover state reveals a rotating conic cyan→violet border, lifts card 6px, rotates the corner arrow 45°, lights an inner ambient halo, and draws a gradient underline on the CTA. Cyan/violet ambient blobs behind the section. Mobile-responsive grid. Reduced-motion respected. |
