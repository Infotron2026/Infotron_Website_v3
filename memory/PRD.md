...
| May 15, 2026 | Tech Ecosystem polished to Portkey-grade: marquee chips are now logos-only (no text) inside larger 132×88 square glass tiles. Logos are served LOCALLY from `/public/tech-logos/*.svg` (Wikimedia / devicon / Simple Icons sources — bypasses Simple Icons trademark-removal 404s for OpenAI/AWS/Azure/Salesforce/ServiceNow/Oracle). Connector lines completely reworked: faint static base lines + bright traveling comet pulses (cyan→violet gradient stroke, 2s loop, `pathLength="100"` normalization, 6 staggered delays). Center panel readouts now bound to a per-frame RAF observer that finds the chip nearest the viewport center in each marquee — when "Anthropic" passes the center of the top row, the panel reads "Currently staffing → Anthropic / AI Platform / Constitutional AI & Claude" with a 0.55s blur-up swap animation. Faster marquee speeds (36s/44s) so the readout changes every ~6 seconds. Mobile-responsive: connectors hidden, panel full-width, chip sizing reduced. |
