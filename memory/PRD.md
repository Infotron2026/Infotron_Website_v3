...
| May 15, 2026 | Technology Ecosystem showcase (Portkey-style) added to Industries section — replaces the two prior tech marquee rows. Industries title/subtitle/background/spacing/industries row remain unchanged. New `<TechEcosystem />` component renders: top marquee (OpenAI · Anthropic · AWS · Azure · Snowflake · Databricks, R→L 60s) using authentic full-color logos via Simple Icons CDN with brand colors, animated dashed SVG connector lines (cyan→violet gradient stroke with pulsing dashoffset) fanning from the row into a floating glassmorphism center panel ("Technology Ecosystem" eyebrow + Playfair italic-gradient heading + subtext + 4 cycling category rows: AI & ML Platforms → Cloud Infrastructure → Enterprise SaaS → DevOps & Containers, each with glow-active state every 2.5s), bottom marquee (Docker · Kubernetes · Salesforce · ServiceNow · SAP · Oracle Cloud, L→R 75s). Mobile: panel full-width, connector SVG hidden, hint text suppressed on <lg. Reduced-motion fallback included. |
