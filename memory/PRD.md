...
| May 15, 2026 | Engagement reveal fix + numbers removed: Removed "0X / 04" labels from all 4 cards. Replaced shared scroll-reveal observer (which was firing too early at rootMargin +120px) with a dedicated IntersectionObserver (threshold 0.25) that triggers `.is-revealed` only when each card is actually visible. Reveal now plays in-view: 0.95s rotateX(-16°)+translateY(32px)+translateZ(-50px) → flat, staggered 140ms per card via inline transitionDelay. Hover micro-lift scoped to `.is-revealed` so it never collides with the entrance animation. Verified via runtime telemetry (opacity 0 → 1.0/0.99/0.95/0.86 stagger captured mid-flight). |
