...
| May 15, 2026 | Industries We Serve — 3-row marquee finalized: row 1 (industries, 12 cards) scrolls LEFT at 55s; row 2 (tech platforms — Oracle, Azure, AWS, ServiceNow, SAP, Salesforce, Workday, VMware, Kubernetes, Snowflake) scrolls RIGHT at 75s; row 3 (tech stacks — DevOps, Data Eng, AI/ML, Cybersecurity, Power BI, Python, React, .NET, Terraform, Databricks) scrolls LEFT at 85s. Added `.tech-card` pill styling (cooler cyan glass tint) to differentiate from indigo industry cards. Edge fade mask + hover-to-pause. |
| May 15, 2026 | Engagement Models section refined: 3 → 4 cards in one horizontal row mirroring Services dropdown order exactly (Managed Services → Capital Projects → Staff Augmentation → Business Consulting). Heading updated to "Four Engagement Models. One Goal." Card padding/icon/title rescaled for 4-up grid; titles use `min-h-[64px]` for cross-card alignment; "Explore →" CTA pinned to bottom of flex column. Added subtle 3D fold-open reveal: `.engagement-card-reveal` with `perspective(1400px)` parent + initial `rotateX(-14deg) translateY(28px) translateZ(-40px)` settling to flat via 0.85s cubic-bezier(0.22, 1, 0.36, 1) easing, staggered 100/200/300/400ms. Restrained motion, enterprise-grade, prefers-reduced-motion respected. |
